...
impl AnkiDeckLoader {
    ...
    fn create_project_template(&self) -> PyResult<()> {
        // Create directory structure
        self.create_dir_structure(&self.project_path)?;
        ...
        self.write_yaml(self.project_path.join("deck.yaml"), &deck_config, "deck.yaml")?;

        // Create sample model
        let model_path = self.project_path.join("models").join("basic_card");
        ...
        self.write_yaml(model_path.join("fields.yaml"), &model_config, "fields.yaml")?;
        ...
        fs::write(
            self.project_path.join("data").join("basic_card.csv"),
