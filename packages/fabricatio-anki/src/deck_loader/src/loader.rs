...
impl AnkiDeckLoader {
    ...
    const DECK_FILE: &'static str = "deck.yaml";
    const FIELDS_FILE: &'static str = "fields.yaml";
    const TEMPLATE_FRONT: &'static str = "front.html";
    const TEMPLATE_BACK: &'static str = "back.html";
    const TEMPLATE_CSS: &'static str = "style.css";
    const SAMPLE_MODEL_NAME: &'static str = "basic_card";
    const SAMPLE_TEMPLATE_NAME: &'static str = "card";
    const SAMPLE_CSV_CONTENT: &'static str =
        "Front,Back\n\"What is the capital of France?\",\"Paris\"\n\"What is 2+2?\",\"4\"";
    const DEFAULT_DECK_NAME: &'static str = "Sample Deck";
    const DEFAULT_DECK_DESCRIPTION: &'static str = "A sample Anki deck";
    const DEFAULT_AUTHOR: &'static str = "Generated by Fabricatio";
    const DEFAULT_FIELD_FRONT: &'static str = "Front";
    const DEFAULT_FIELD_BACK: &'static str = "Back";
    ...
    fn load_templates(&self, templates_path: &Path) -> Vec<TemplateConfig> {
        ...
        fs::read_dir(templates_path)
            ...
            .for_each(|entry| {
                ...
                let front_html = fs::read_to_string(template_path.join(Self::TEMPLATE_FRONT))
                    .unwrap_or_default();
                let back_html = fs::read_to_string(template_path.join(Self::TEMPLATE_BACK))
                    .unwrap_or_default();
                let style_css = fs::read_to_string(template_path.join(Self::TEMPLATE_CSS))
                    .unwrap_or_default();
    ...
    fn collect_all_media_files(&self, model_names: &[String]) -> Result<Vec<String>, String> {
        ...
        let global_media_path = self.project_path.join(Self::MEDIA_DIR);
        all_media_files.extend(
    ...
    fn create_dir_structure(&self, path: &Path) -> Result<(), String> {
        let dirs = [Self::MODELS_DIR, Self::DATA_DIR, Self::MEDIA_DIR];
        for dir in &dirs {
    ...
    pub fn load_deck_config(&self) -> Result<DeckConfig, String> {
        let deck_config_path = self.project_path.join(Self::DECK_FILE);
        self.read_yaml(deck_config_path, Self::DECK_FILE)
    }

    pub fn load_model_data(&self, model_name: &str) -> Result<ModelData, String> {
        let model_path = self.project_path.join(Self::MODELS_DIR).join(model_name);

        // Load model config
        let fields_path = model_path.join(Self::FIELDS_FILE);
        let config: ModelConfig = self.read_yaml(
            fields_path,
            &format!("{} for model {}", Self::FIELDS_FILE, model_name),
        )?;
        ...
        // Load media files
        let media_files = self.collect_files_from_dir(&model_path.join(Self::MEDIA_DIR));
    ...
    pub fn get_available_models(&self) -> Result<Vec<String>, String> {
        let models_path = self.project_path.join(Self::MODELS_DIR);
    ...
    pub fn create_project_template(
        ...
        let deck_config = DeckConfig {
            name: deck_name.unwrap_or_else(|| Self::DEFAULT_DECK_NAME.to_string()),
            description: deck_description
                .unwrap_or_else(|| Self::DEFAULT_DECK_DESCRIPTION.to_string()),
            deck_id: std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_else(|_| std::time::Duration::from_secs(0))
                .as_secs() as i64,
            author: author.or(Some(Self::DEFAULT_AUTHOR.to_string())),
        };
        loader.write_yaml(
            project_path.join(Self::DECK_FILE),
            &deck_config,
            Self::DECK_FILE,
        )?;

        // Create sample model
        let model_name = model_name.unwrap_or_else(|| Self::SAMPLE_MODEL_NAME.to_string());
        let model_path = project_path.join(Self::MODELS_DIR).join(&model_name);
        ...
        let model_config = ModelConfig {
            ...
            fields: fields.unwrap_or_else(|| {
                vec![
                    Self::DEFAULT_FIELD_FRONT.to_string(),
                    Self::DEFAULT_FIELD_BACK.to_string(),
                ]
            }),
        };
        loader.write_yaml(
            model_path.join(Self::FIELDS_FILE),
            &model_config,
            Self::FIELDS_FILE,
        )?;

        // Create sample template
        let template_path = model_path
            .join("templates")
            .join(Self::SAMPLE_TEMPLATE_NAME);
        fs::create_dir_all(&template_path)
            .map_err(|e| format!("Failed to create template directory: {}", e))?;

        let template_files = [
            (Self::TEMPLATE_FRONT, "{{Front}}"),
            (
                Self::TEMPLATE_BACK,
                "{{FrontSide}}\n\n<hr id=\"answer\">\n\n{{Back}}",
            ),
