...
class RequiredStep(Action):
    """Action with a required field that has NO default — value must come from a wire."""
...
async def test_wired_required_field_without_default_is_kept() -> None:
    ...
    wf = _wf(
        [
            _node("a", "EchoStep", {"value": "X"}),
            _node("b", "RequiredStep", {}),
            _node("c", "PickStep", {"key": "req"}),
        ],
        [
            {"source": "a", "source_handle": "echo", "target": "b", "target_handle": "required_field"},
            {"source": "b", "source_handle": "req", "target": "c", "target_handle": "observe"},
        ],
    )
    out, task = await _run(wf)
    assert task.is_finished()
...
@pytest.mark.asyncio
async def test_unwired_required_field_without_default_still_skips() -> None:
    """A required field with no default and no wire keeps the node-skip backstop."""
    wf = _wf(
        [
            _node("a", "RequiredStep", {}),
        ],
        [],
    )
    out, task = await _run(wf)
