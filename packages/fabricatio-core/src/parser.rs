...
impl TextCapturer {
    fn cap(&self, text: &str) -> Option<String> {
        self.reg.captures(text)
            .map(Self::get_fist)
    }


    fn cap_all(&self, text: &str) -> Vec<String> {
        self.reg.captures_iter(text)
            .map(Self::get_fist)
            .collect()
    }
    #[staticmethod]
...
impl JsonParser {
    ...
    pub fn capture_all(&self, text: &str, fix: bool) -> PyResult<Vec<String>> {
        if fix {
            self.capturer.cap_all(text).into_iter().map(Self::fix_json_string).try_collect()
        } else {
            Ok(self.capturer.cap_all(text))
        }
    }
    #[pyo3(signature=(text, fix=true))]
    pub fn convert<'a>(&self, python: Python<'a>, text: &str, fix: bool) -> PyResult<Bound<'a, PyAny>> {
        pythonize(python, &Self::deserialize::<Value>(text, fix)?).into_pyresult()
    }


    #[pyo3(signature=(text, fix=true))]
    pub fn convert_all<'a>(&self, python: Python<'a>, text: &str, fix: bool) -> PyResult<Vec<Bound<'a, PyAny>>> {
        self.capture_all(text, fix)?
            .into_iter()
            .map(|json|
                pythonize(python, &Self::deserialize::<Value>(json, fix)?).into_pyresult()
            )
    ...
    pub fn validate_list<'a>(&self, python: Python<'a>, text: &str,

                             #[gen_stub(override_type(type_repr = "None|typing.Type[_T]"))]
                             elements_type: Option<&Bound<PyType>>,
                             length: Option<usize>, fix: bool) -> PyResult<Option<Bound<'a, PyList>>> {
        ...
        if let Ok(val_list) = val.cast_into_exact::<PyList>() &&
            ...
            (elements_type.is_none() || elements_type.is_some_and(|t|
    ...
    pub fn validate_dict<'a>(&self, python: Python<'a>, text: &str,

                             #[gen_stub(override_type(type_repr = "None|typing.Type[_K]"))]
                             key_type: Option<&Bound<PyType>>,
                             #[gen_stub(override_type(type_repr = "None|typing.Type[_V]"))]
                             value_type: Option<&Bound<PyType>>,
                             length: Option<usize>,
                             fix: bool) -> PyResult<Option<Bound<'a, PyDict>>> {
        let val = self.convert(python, text, fix)?;
        if let Ok(val_dict) = val.cast_into_exact::<PyDict>() &&
            (length.is_none() || length.is_some_and(|l| val_dict.len() == l))
        {
            let key_check = key_type.is_none() || key_type.is_some_and(|t|
                val_dict.keys().iter().all(|item| item.is_instance(t).unwrap_or(false))
            );

            let value_check = value_type.is_none() || value_type.is_some_and(|t|
...
pub(crate) fn register(_: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add("json_parser", JsonParser::with_capturer(TextCapturer::capture_code_block(Some("json"))?))?;
    m.add("python_parser", TextCapturer::capture_code_block(Some("python"))?)?;
    m.add("generic_parser", TextCapturer::capture_generic_block("String")?)?;
    m.add("snippet_parser", TextCapturer::capture_snippet(">>>>>", "<<<<<")?)?;
    Ok(())
}
