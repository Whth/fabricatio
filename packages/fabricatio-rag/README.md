...
`[ext.rag]` table in `fabricatio.toml`, equivalently under
`[tool.fabricatio.ext.rag]` in `pyproject.toml`, or via
`FABRICATIO_EXT__RAG__<FIELD_UPPER>` environment variables.
