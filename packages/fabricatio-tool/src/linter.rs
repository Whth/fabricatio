...
impl<'a> Visitor for LinterVisitor<'a> {
    fn visit_stmt(&mut self, node: Stmt<TextRange>) {
        ...
        if let Some(violation) = node.as_import_stmt().map(|stmt| check_import(&Stmt::Import(stmt.clone()), self.config)) {
    ...
    fn visit_expr(&mut self, node: Expr<TextRange>) {
        if let Some(violation) = node.as_call_expr().map(|expr| check_call(&Expr::Call(expr.clone()), self.config)) {
            if let Some(violation) = violation {
                self.violations.push(violation);
            }
        }
...
pub fn gather_violations<S: AsRef<str>>(source: S, config: LinterConfig) -> Result<Vec<String>, String> {
