...
async def main() -> None:
    """Run the novel-writing workflow twice: once in English, once in Chinese. Each task configures the novel outline, output path, font, cover image, language, and chapter guidance, then delegates to the DebugNovelWorkflow to produce an EPUB file."""
    # Dispatch the task
    path = await (
        Task(
