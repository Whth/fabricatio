"""Demonstrates Anki deck generation from CSV question banks. Shows a two-step pipeline: first generate a deck from raw topics, then enrich it with topic analysis before re-generating for better quality."""
...
class DeckGen(Action, GenerateDeck):
    """Generate an Anki deck from a CSV question bank. Reads column names from CSV, uses LLM to create flashcards matching the requirement specification, saves the deck, and imports CSV data."""
